...
describe "Statistrano::Deployment::MultiTarget integration", :integration do

  context "with a single target" do
    before :each do
      Given.fixture "base"
      define_deployment "single_target", :multi_target do
      ...
      reenable_rake_tasks
      Rake::Task["single_target:deploy"].invoke
  ...
  context "with multiple targets" do
    ...
    before :each do
      Given.fixture "base"
      define_deployment "multi_target", :multi_target do
        build_task "remote:copy"
        local_dir  "build"
        remote     "localhost"
        remote_dir File.join( Dir.pwd, "deployment" )
      ...
      allow( Time ).to receive(:now).and_return(1372020000)
      reenable_rake_tasks
      Rake::Task["multi_target:deploy"].invoke
